...
class NumericalCSVFile(CSVFile):
    ...
    def get_data(self, start, end):
        ...
        # Setto start e send se vengono passati
        start_index = start if (type(start) is int and start >= 0) else 0
        end_index = end if (type(start) is int and end >= 0) else len(string_data)

        
        # Ciclo su tutte le "righe" corrispondenti al file originale 
        for string_row in string_data[start_index : end_index]:
...
print('Dati contenuti nel file: "{}"'.format(mio_file_numerico.get_data(None, None)))
